...
// 环境变量 PERFECT_TWEET_CONFIG 可指定自定义配置文件（多 Agent 实例共享同一份配置）
export const CONFIG_PATH = process.env.PERFECT_TWEET_CONFIG
    || path.join(os.homedir(), '.newmax', 'skills', 'perfect-tweet', 'config.json');
...
export function validatePatch(patch) {
    ...
    for (const [key, value] of Object.entries(patch || {})) {
        ...
        if (rule === 'boolean') {
            ...
            // 网页导出的 JSON 里是真实 null（JSON null），CLI 传参时则是字符串 "null"；
            // 两者以及空串一律归一化为真 null，避免 String(null) 产出字符串 "null"
            clean[key] = value === null || value === 'null' || value === '' ? null : String(value);
